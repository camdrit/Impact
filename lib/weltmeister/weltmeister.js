...
wm.Weltmeister = ig.Class.extend({	
	...
	init: function() {
		...
		document.getElementById('levelLoad').addEventListener( 'click', this.showLoadDialog.bind(this) );
		document.getElementById('levelNew').addEventListener( 'click', this.showNewDialog.bind(this) );
		...
		document.getElementById('levelSaveAs').addEventListener( 'click', this.saveDialog.open.bind(this.saveDialog) );
		document.getElementById('levelSave').addEventListener( 'click', this.saveQuick.bind(this) );
		...
		this.entities = new wm.EditEntities( document.getElementById('layerEntities') );
		
		// $('#layers').sortable({
		// 	update: this.reorderLayers.bind(this)
		// });
		document.getElementById('layers').setAttribute('style',
		`
		...
		document.getElementById('buttonAddLayer').addEventListener( 'click', this.addLayer.bind(this) );
		document.getElementById('buttonRemoveLayer').addEventListener( 'click', this.deleteLayerDialog.open.bind(this.deleteLayerDialog) );
		document.getElementById('buttonSaveLayerSettings').addEventListener( 'click', this.saveLayerSettings.bind(this) );
		document.getElementById('reloadImages').addEventListener( 'click', ig.Image.reloadCache );
		document.getElementById('layerIsCollision').addEventListener( 'change', this.toggleCollisionLayer.bind(this) );
		
		document.querySelector('input#toggleSidebar').addEventListener('click', function() {
			...
			var toggleSidebar = document.querySelector('input#toggleSidebar');
			if (toggleSidebar.classList.contains('active')) {
				toggleSidebar.classList.remove('active');
			} else toggleSidebar.classList.add('active');
		});
		...
		document.getElementById('canvas').addEventListener('mousedown', function(){
			var selected = document.querySelectorAll('input:focus');
		...
		if( wm.config.loadLastLevel ) {
			var path = localStorage.getItem('wmLastLevel');
			if( path && path != "null" ) {
				this.load( null, path )
			}
		}
	...
	uikeydown: function( event ) {
		...
		if( key.match(/^\d$/) ) {
			...
			var name = document.querySelector('#layers divlayer:nth-child('+index+') span.name').innerText();
	...
	setWindowTitle: function() {
		...
		document.querySelector('span.headerTitle').innerText = this.fileName;
		document.querySelector('span.unsavedTitle').innerText = this.modified ? '*' : '';
	},
	...
	zoom: function( delta ) {
		...
		document.getElementById('zoomIndicator').text( wm.config.view.zoom + 'x' ).stop(true,true).show().delay(300).fadeOut();
	...
	loadNew: function() {
		localStorage.setItem( 'wmLastLevel', null );
	...
	load: function( dialog, path ) {
		...
		req.open('GET', path + '?nocache=' + Math.random(), false );
		req.onload = () => {
			if (req.status === 200) {
				this.loadResponse.bind(this);
			} else localStorage.setItem('wmLastLevel', null);
		};
		req.send();
		// var req = $.ajax({
	...
	loadResponse: function( data ) {
		localStorage.setItem( 'wmLastLevel', this.filePath );
		...
		this.reorderLayers();
		document.getElementById('layers').sortable('refresh');
	...
	save: function( dialog, path ) {
		...
		var req = new XMLHttpRequest();
		req.open('POST', wm.config.api.save, false);
		req.onload = () => {
			if (req.status === 200) {
	...
	updateLayerSettings: function( ) {
		document.getElementById('layerName').val( this.activeLayer.name );
		document.getElementById('layerTileset').val( this.activeLayer.tilesetName );
		document.getElementById('layerTilesize').val( this.activeLayer.tilesize );
		document.getElementById('layerWidth').val( this.activeLayer.width );
		document.getElementById('layerHeight').val( this.activeLayer.height );
		document.getElementById('layerPreRender').prop( 'checked', this.activeLayer.preRender );
		document.getElementById('layerRepeat').prop( 'checked', this.activeLayer.repeat );
		document.getElementById('layerLinkWithCollision').prop( 'checked', this.activeLayer.linkWithCollision );
		document.getElementById('layerDistance').val( this.activeLayer.distance );
	},
	
	
	saveLayerSettings: function() {
		var isCollision = document.getElementById('layerIsCollision').prop('checked');
		
		var newName = document.getElementById('layerName').val();
		var newWidth = Math.floor(document.getElementById('layerWidth').val());
		var newHeight = Math.floor(document.getElementById('layerHeight').val());
		
		if( newWidth != this.activeLayer.width || newHeight != this.activeLayer.height ) {
			this.activeLayer.resize( newWidth, newHeight );
		}
		this.activeLayer.tilesize = Math.floor(document.getElementById('layerTilesize').val());
		...
		if( isCollision ) {
			...
			var newTilesetName = document.getElementById('layerTileset').val();
			if( newTilesetName != this.activeLayer.tilesetName ) {
				this.activeLayer.setTileset( newTilesetName );
			}
			this.activeLayer.linkWithCollision = document.getElementById('layerLinkWithCollision').prop('checked');
			this.activeLayer.distance = document.getElementById('layerDistance').val();
			this.activeLayer.repeat = document.getElementById('layerRepeat').prop('checked');
			this.activeLayer.preRender = document.getElementById('layerPreRender').prop('checked');
		}
	...
	setActiveLayer: function( name ) {
		...
		document.getElementById('layerIsCollision').checked = (name == 'collision');
		
		if( name == 'entities' ) {
			this.fadeOut( document.getElementById('layerSettings') );
		}
		else {
			this.entities.selectEntity( null );
			this.toggleCollisionLayer();
			document.getElementById('layerSettings')
...
wm.Weltmeister.getMaxHeight = function() {
	return window.innerHeight - document.getElementById('headerMenu').clientHeight;
};
