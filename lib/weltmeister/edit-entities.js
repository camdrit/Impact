...
wm.EditEntities = ig.Class.extend({
	...
	init: function( div ) {
		...
		for (var i = 0; i < this.div.querySelectorAll('.visible').length; i++) {
			var visible = this.div.querySelectorAll('.visible')[i];
			visible.addEventListener( 'mousedown', this.toggleVisibilityClick.bind(this) );
		}
		
		this.menu = document.getElementById('entityMenu');
		this.importEntityClass( wm.entityModules );
		this.entityDefinitions = document.getElementById('entityDefinitions');
		
		document.getElementById('entityKey').addEventListener( 'keydown', function(ev){ 
			if( ev.which == 13 ){ 
				document.getElementById('entityValue').focus(); 
				return false;
			}
			return true;
		});
		document.getElementById('entityValue').addEventListener( 'keydown', this.setEntitySetting.bind(this) );
	},
	...
	selectEntity: function( entity ) {
		if( entity && entity != this.selectedEntity ) {
			...
			document.getElementById('entitySettings').classList.remove('fadeIn');
			document.getElementById('entitySettings').classList.add('fadeOut');
			setTimeout(() => {
				this.loadEntitySettings();
				document.getElementById('entitySettings').classList.remove('fadeOut');
				document.getElementById('entitySettings').classList.add('fadeIn');
			}, 100);
			...
			document.getElementById('entitySettings').classList.remove('fadeIn');
			document.getElementById('entitySettings').classList.add('fadeOut');
			document.getElementById('entityKey').blur();
			document.getElementById('entityValue').blur();
		}
		
		this.selectedEntity = entity;
		document.getElementById('entityKey').value = '';
	...
	toggleVisibility: function() {
		this.visible ^= 1;
		if( this.visible ) {
			var children = this.div.querySelectorAll('.visible');
			for (var i = 0; i < children.length; i++) {
				children[i].classList.add('checkedVis');
			}
		} else {
			var children = this.div.querySelectorAll('.visible');
			for (var i = 0; i < children.length; i++) {
				children[i].classList.remove('checkedVis');
			}
		}
		ig.game.draw();
	},
